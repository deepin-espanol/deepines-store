...
OLDOLDSTABLEDIST="apricot"
OLDOLDSTABLEREPO="## Generated by deepin-installer
...
NOT_SUPPORTED="This operating system is not Deepin 20, Deepin 23 or Deepin 25."
ONLY_COMPATIBLE="  This repair script is only for Deepin 20, Deepin 23 and Deepin 25.
...
es*)
	...
	NOT_SUPPORTED="Este sistema operativo no es Deepin 20, Deepin 23 o Deepin 25."
	ONLY_COMPATIBLE=" Este script de reparación es sólo para Deepin 20, Deepin 23 y Deepin 25.
...
Main() {
	...
	if [ "$DIST_ID" = "Deepin" ]; then
		case $REL_NUM in
		20 | 20.*)
			CheckRepos "/deepin/|/deepin-20-beta/" $OLDOLDSTABLEDIST || WantRestoreRepos "20" "$OLDOLDSTABLEREPO"
			;;
		"23 Nightly" | 23 | 23.*)
			CheckRepos "/beige/|/dde-nightly/" $OLDSTABLEDIST || WantRestoreRepos "23" "$OLDSTABLEREPO"
			;;
		25)
			CheckRepos "/beige/" $STABLEDIST || WantRestoreRepos "25" "$STABLEREPO"
			;;
		*) UnsupportedOS ;;
		esac
	else
